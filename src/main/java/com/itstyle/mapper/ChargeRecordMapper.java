...
public interface ChargeRecordMapper extends JpaRepository<ChargeRecord, Long>, JpaSpecificationExecutor<ChargeRecord> {
    ...
    @Query(value = "select SUM(cn.fee) AS total_fee , SUM(cn.receivable_fee) AS total_receivable_fee , SUM(cn.discount_amount) AS total_discount_amount " +
            ...
            "AND if(?4 is not null,charge_personnel=?4 , 1 = 1 ) " +
            "AND if(?5 is not null,time>=?5 , 1 = 1 ) " +
            "AND if(?6 is not null,time<=?6 , 1 = 1 ) ", nativeQuery = true)
    List<Object[]> statistics(Integer chargeType, Integer carType, Integer carRealType, String chargePersonnel, Long startTime, Long endTime);
}
